...
fn full_application_shadow_fits_each_layout() {
    let mut eight = Ra8m2Granule8::to_shadow_ranges(0, usize::MAX);
    assert_eq!(
        eight.next(),
        Some(Shadow {
    ...
    let mut sixteen = Ra8m2Granule16::to_shadow_ranges(0, usize::MAX);
    assert_eq!(
        sixteen.next(),
        Some(Shadow {
...
fn sram_only_layout_keeps_all_shadow_out_of_dtcm() {
    let mut pieces = Ra8m2Granule8Sram::to_shadow_ranges(0, usize::MAX);
    assert_eq!(
        pieces.next(),
        Some(Shadow {
    ...
    assert!(
        Ra8m2Granule8Sram::to_shadow_ranges(0x2000_0000, 0x20000)
            .next()
            .is_none()
    );
}
    ...
    for boundary in [
        ...
        for addr in boundary - 32..=boundary + 32 {
            for size in 0..=32 {
                ...
                let mut next = first;
                for part in P::to_shadow_ranges(addr, size) {
                    ...
                    assert!(
                        P::to_shadow_ranges(0, usize::MAX)
...
fn wrapping_ranges_have_no_shadow() {
    assert!(
        Ra8m2Granule8Sram::to_shadow_ranges(usize::MAX, 2)
            .next()
            .is_none()
    );
    assert!(
        Ra8m2Granule8::to_shadow_ranges(usize::MAX, 2)
            .next()
            .is_none()
    );
    assert!(
        Ra8m2Granule16::to_shadow_ranges(0x2200_0000, usize::MAX)
            .next()
            .is_none()
    );
}
