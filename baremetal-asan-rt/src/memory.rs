...
use crate::{access::check_range, layout::Layout};
...
///
/// # Safety
/// Shadow RAM must satisfy `Layout::to_slices` throughout the check.
/// For nonzero `size`, `src` must be readable and `dst` writable for `size` bytes.
pub unsafe fn memcpy<L: Layout>(dst: *mut c_void, src: *const c_void, size: usize) -> *mut c_void {
    if size == 0 {
        return dst;
    }
    unsafe { check_range::<L>(src as usize, size, false) };
    unsafe { check_range::<L>(dst as usize, size, true) };
    if !ptr::eq(dst, src) {
...
/// Shadow RAM must satisfy `Layout::to_slices` throughout the check.
/// For nonzero `size`, `src` must be readable and `dst` writable for `size` bytes.
pub unsafe fn memmove<L: Layout>(dst: *mut c_void, src: *const c_void, size: usize) -> *mut c_void {
    if size != 0 {
        unsafe { check_range::<L>(src as usize, size, false) };
        unsafe { check_range::<L>(dst as usize, size, true) };
...
pub unsafe fn memset<L: Layout>(dst: *mut c_void, value: c_int, size: usize) -> *mut c_void {
    if size != 0 {
        unsafe { check_range::<L>(dst as usize, size, true) };
...
/// Export checked memcpy, memmove, and memset for the supplied layout.
#[macro_export]
macro_rules! export_asan_memory {
    ($layout:ty) => {
        ...
        pub unsafe extern "C" fn __asan_memcpy(
            ...
            unsafe { $crate::memory::memcpy::<$layout>(dst, src, size) }
        }
        ...
        pub unsafe extern "C" fn __asan_memmove(
            ...
            unsafe { $crate::memory::memmove::<$layout>(dst, src, size) }
        }
        ...
        pub unsafe extern "C" fn __asan_memset(
            ...
            unsafe { $crate::memory::memset::<$layout>(dst, value, size) }
        }
    };
}
