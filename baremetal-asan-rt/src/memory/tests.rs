...
fn memcpy_copies_only_the_requested_bytes_and_returns_destination() {
    ...
    assert_eq!(
        unsafe { __asan_memcpy(target, src.as_ptr().cast(), src.len()) },
        target
    );
...
fn memmove_supports_overlap_in_both_directions() {
    ...
    assert_eq!(
        unsafe { __asan_memmove(p.add(2).cast(), p.cast(), 8) },
        unsafe { p.add(2).cast() }
    );
    ...
    let p = bytes.as_mut_ptr();
    assert_eq!(
        unsafe { __asan_memmove(p.cast(), p.add(2).cast(), 8) },
        p.cast()
    );
...
fn memset_truncates_to_a_byte_and_preserves_surrounding_bytes() {
    for value in [-1, 0x123] {
        ...
        assert_eq!(unsafe { __asan_memset(dst, value, 7) }, dst);
...
fn identical_copy_ranges_are_allowed() {
    ...
    assert_eq!(unsafe { __asan_memcpy(p, p, bytes.len()) }, p);
    assert_eq!(unsafe { __asan_memmove(p, p, bytes.len()) }, p);
...
fn zero_length_operations_accept_null_pointers() {
    let p = ptr::null_mut();
    assert_eq!(unsafe { __asan_memcpy(p, p, 0) }, p);
    assert_eq!(unsafe { __asan_memmove(p, p, 0) }, p);
    assert_eq!(unsafe { __asan_memset(p, -1, 0) }, p);
}
